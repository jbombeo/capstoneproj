...
export default function GoodMoral() {
  ...
  const [docs, setDocs] = useState<DocumentRequest[]>(props.docs || []);
  ...
  // ==========================
  // Filter Good Moral Requests
  // ==========================
  const goodMoralDocs = docs.filter(doc => doc.document_type?.name === "Certificate of Good Moral");
  const filteredDocs = goodMoralDocs.filter(req =>
  ...
  // ==========================
  return (
    <AppLayout breadcrumbs={[{ title: "Certificate of Good Moral", href: "/good-moral" }]}>
      <Head title="Certificate of Good Moral" />
      <Toaster position="top-right" />

      {/* HEADER */}
      <div className="flex justify-between items-center mb-8 bg-green-600 text-white shadow p-6">
        <h1 className="text-2xl font-bold">Certificate of Good Moral Requests</h1>
        <Button
          ...
          <div className="grid grid-cols-3 gap-6 mt-10">
            ...
            <div className="col-span-1 pr-4 text-sm">
              <div className="p-4 rounded-lg shadow-md border border-blue-700 bg-blue-50">
                <h2 className="font-bold text-blue-800 text-center mb-4 text-base">Barangay Officials</h2>
                {punongBarangay && (
                  <>
                ...
                {skChairman && <p className="mt-2 font-bold text-[15px]">{skChairman.complete_name} - SK Chairman</p>}
                {secretary && <p className="mt-2 font-bold text-[15px]">{secretary.complete_name} - Secretary</p>}
                {treasurer && <p className="mt-2 font-bold text-[15px]">{treasurer.complete_name} - Treasurer</p>}
              </div>
            </div>
            ...
            <div className="col-span-2 pl-6 text-justify text-[15px] relative">
              ...
              <div className="relative z-10">
                <p className="mt-2">
                  This is to certify that <span className="font-bold underline">{printDoc.resident.first_name} {printDoc.resident.last_name}</span> is known to be of good moral character and a law-abiding citizen of Barangay Iponan, Cagayan de Oro City.
                </p>
                <p className="mt-4">This certification is issued upon request for the purpose of <span className="font-bold underline">{printDoc.purpose}</span>.</p>
                <p className="mt-4">Given this <span className="underline">{formattedDate}</span> at Barangay Iponan, Cagayan de Oro City, Misamis Oriental.</p>
                {punongBarangay && (
                ...
                <p className="mt-2 text-center text-sm text-gray-600">Scan QR code to verify release status</p>
              </div>
            </div>
          </div>
        </div>
